...
const SearchResult: React.FC<ResultProps> = ({ data }) => {
  return (
    <Link to={`/story/${data.username}/page/1`} className="flex flex-row justify-between items-center h-20 w-full mb-4 px-6 py-3 bg-secondary rounded-xl hover:shadow-lg hover:scale-105 transition duration-300 ease-in-out">
      <div className="flex flex-row h-16 justify-center items-center overflow-hidden">
        <img src={`https://ojfessrwpkkqljbturrx.supabase.co/storage/v1/object/public/uploads/${data.image}`} className={data.image !== null ? "hidden md:flex h-14 w-24 object-cover rounded-xl mr-6" : "hidden"} />
